...
class TestFindPodCidr:
    ...
    def test_finds_100x_cidr(self):
        cidrs = ["10.1.0.0/16", "192.168.1.0.0/16"]
        assert _find_pod_cidr(cidrs) == "192.168.1.0.0/16"
    ...
    def test_returns_first_match(self):
        cidrs = ["192.168.0.0.0/16", "192.168.1.0.0/16"]
        assert _find_pod_cidr(cidrs) == "192.168.0.0.0/16"

    def test_empty_list(self):
        assert _find_pod_cidr([]) is None

    def test_invalid_cidr_skipped(self):
        cidrs = ["not-a-cidr", "192.168.1.0.0/16"]
        assert _find_pod_cidr(cidrs) == "192.168.1.0.0/16"
...
class TestLookupEksPodCidr:
    ...
    async def test_finds_pod_cidr_from_vpc(self):
        ...
        assert result == "192.168.1.0.0/16"
...
class TestLookupVpcCidrs:
    ...
    async def test_returns_primary_and_secondary(self):
        ...
        neo4j.query = AsyncMock(return_value=[{
            ...
            "secondary": ["192.168.1.0.0/16"],
        }])
        result = await lookup_vpc_cidrs(neo4j, "arn:eks:cluster")
        assert result == ["10.0.32.0/20", "192.168.1.0.0/16"]
...
class TestPickSamplePodIp:
    ...
    def test_slash_16(self):
        ip = pick_sample_pod_ip("192.168.1.0.0/16")
        assert ip == "192.168.1.0.1"

    def test_slash_24(self):
        ip = pick_sample_pod_ip("192.168.1.1.0/24")
        assert ip == "192.168.1.1.1"
    ...
    def test_slash_32_returns_host(self):
        ...
        assert pick_sample_pod_ip("192.168.1.0.1/32") == "192.168.1.0.1"

    def test_slash_31_returns_first(self):
        # /31 has 2 hosts (point-to-point)
        ip = pick_sample_pod_ip("192.168.1.0.0/31")
        assert ip == "192.168.1.0.0"
